...
type ShoppingCategory = 'All' | 'Nehru Street' | 'Anna Salai' | 'Local Street Markets' | 'Malls' | 'Shopping Malls' | 'Supermarkets' | 'Special Areas' | 'Auroville Shopping' | 'Handicraft Stores' | 'Souvenir Shops';
...
const SHOPPING_CATEGORIES: { id: ShoppingCategory; label: string; labelTamil: string; image: string }[] = [
  { 
    id: 'Nehru Street', 
    label: 'Nehru Street', 
    labelTamil: 'நேரு தெரு', 
    image: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800' 
  },
  { 
    id: 'Anna Salai', 
    label: 'Anna Salai', 
    labelTamil: 'அண்ணா சாலை', 
    image: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800' 
  },
  { 
    id: 'Local Street Markets', 
    label: 'Local Street Markets', 
    labelTamil: 'உள்ளூர் தெரு சந்தைகள்', 
  ...
  { 
    ...
    labelTamil: 'ஷாப்பிங் மால்', 
    image: 'https://images.unsplash.com/photo-1555529669-e69e7aa0ba9a?w=800' 
  },
  { 
    id: 'Supermarkets', 
    label: 'Supermarkets', 
    labelTamil: 'சூப்பர் மார்க்கெட்', 
    image: 'https://images.unsplash.com/photo-1555529669-e69e7aa0ba9a?w=800' 
  },
  { 
    id: 'Special Areas', 
    label: 'Special Areas', 
    labelTamil: 'சிறப்பு பகுதிகள்', 
    image: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800' 
  },
  { 
    id: 'Auroville Shopping', 
    label: 'Auroville Shopping', 
    labelTamil: 'ஆரோவில் ஷாப்பிங்', 
    image: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800' 
  },
  { 
    id: 'Handicraft Stores', 
    label: 'Handicraft Stores', 
    labelTamil: 'கைவினை கடைகள்', 
    image: 'https://images.unsplash.com/photo-1555529669-e69e7aa0ba9a?w=800' 
  },
  { 
    id: 'Souvenir Shops', 
    label: 'Souvenir Shops', 
    labelTamil: 'நினைவுச்சின்ன கடைகள்', 
    image: 'https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=800' 
  },
];
...
export default function ShoppingScreen({ navigation }: ShoppingScreenProps) {
  ...
  const [filteredPlaces, setFilteredPlaces] = useState<ShoppingPlace[]>([]);
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const data = shoppingData as ShoppingPlace[];
        if (isMounted) {
  ...
  const handleCategoryPress = (category: ShoppingCategory) => {
    setSelectedCategory(category);
    setShowCategoryView(false);
  };

  const handlePlacePress = (place: ShoppingPlace) => {
    ...
    navigation?.navigate('PlaceDetails', { place: placeForDetails, shoppingPlace: place });
  };

  const getDisplayName = (place: ShoppingPlace): string => {
  ...
  const getDisplayDescription = (place: ShoppingPlace): string => {
    const desc = place.description;
    if (!desc) return 'Shopping place in Pondicherry';
    
    let fullDesc = '';
    if (desc.specialFeatures) fullDesc += desc.specialFeatures + '\n\n';
    if (desc.whatToBuy && desc.whatToBuy.length > 0) {
      fullDesc += 'What to buy: ' + desc.whatToBuy.join(', ') + '\n\n';
    }
    if (desc.visitorPurpose) fullDesc += desc.visitorPurpose;
    
    return fullDesc || 'Shopping place in Pondicherry';
  };
  ...
  const renderCategoryCard = ({ item, index }: { item: typeof SHOPPING_CATEGORIES[0]; index: number }) => (
    <TouchableOpacity
      style={[
        styles.categoryCard,
  ...
  const renderPlaceCard = ({ item, index }: { item: ShoppingPlace; index: number }) => (
    <TouchableOpacity
      style={[
        styles.placeCard,
        index % 2 === 0 ? styles.cardLeft : styles.cardRight,
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading shopping places...</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {showCategoryView ? 'Shopping Places' : selectedCategory === 'All' ? 'All Places' : selectedCategory}
        </Text>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.filterOptions}>
                {(['All', ...SHOPPING_CATEGORIES.map(c => c.id)] as ShoppingCategory[]).map((cat) => (
                  <TouchableOpacity
                    key={cat}
                    style={[
                      styles.filterOption,
