...
// Import photoshoot data
import photoshootData from '../data/photoshoot.json';

const STATUSBAR_HEIGHT = Platform.OS === 'android' ? StatusBar.currentHeight || 24 : 0;

type Language = 'English' | 'Tamil';
type PhotoshootCategory = 'All' | 'White Town Streets' | 'Auroville' | 'Beachside Locations' | 'Heritage Buildings' | 'Sunrise / Sunset Spots';

interface PhotoshootPlace {
  id: string;
  name: string;
  nameTamil?: string;
  category: string;
...
const PHOTOSHOOT_CATEGORIES: { id: PhotoshootCategory; label: string; labelTamil: string; image: string }[] = [
  { 
    id: 'White Town Streets', 
    label: 'White Town Streets', 
    labelTamil: 'வெள்ளை நகர தெருக்கள்', 
  ...
  { 
    id: 'Beachside Locations', 
    label: 'Beachside Locations', 
    labelTamil: 'கடற்கரை இடங்கள்', 
    image: 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=800' 
  },
  { 
    id: 'Heritage Buildings', 
    label: 'Heritage Buildings', 
    labelTamil: 'பாரம்பரிய கட்டிடங்கள்', 
    image: 'https://images.unsplash.com/photo-1487958449943-2429e8be8625?w=800' 
  },
  { 
    id: 'Sunrise / Sunset Spots', 
    label: 'Sunrise / Sunset', 
    labelTamil: 'சூரிய உதயம் / மறைவு', 
    image: 'https://images.unsplash.com/photo-1509316785289-025f5b846b35?w=800' 
  },
];

export default function PhotoshootScreen({ navigation }: PhotoshootScreenProps) {
  const [places, setPlaces] = useState<PhotoshootPlace[]>([]);
  const [filteredPlaces, setFilteredPlaces] = useState<PhotoshootPlace[]>([]);
  const [loading, setLoading] = useState(true);
  const [selectedCategory, setSelectedCategory] = useState<PhotoshootCategory>('All');
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setLoading(true);
      try {
        const data = photoshootData as PhotoshootPlace[];
        if (isMounted) {
          setPlaces(Array.isArray(data) ? data : []);
          setFilteredPlaces(Array.isArray(data) ? data : []);
        }
      } catch (error) {
        console.error('Error loading photoshoot places:', error);
        if (isMounted) {
          setPlaces([]);
          setFilteredPlaces([]);
        }
  ...
  const handleCategoryPress = (category: PhotoshootCategory) => {
    setSelectedCategory(category);
    setShowCategoryView(false);
  };

  const handlePlacePress = (place: PhotoshootPlace) => {
    ...
    navigation?.navigate('PlaceDetails', { place: placeForDetails, photoshootPlace: place });
  };

  const getDisplayName = (place: PhotoshootPlace): string => {
  ...
  const getDisplayDescription = (place: PhotoshootPlace): string => {
    const desc = place.description;
    if (!desc) return 'Photoshoot location in Pondicherry';
    
    let fullDesc = '';
    if (desc.specialFeatures) fullDesc += desc.specialFeatures + '\n\n';
    if (desc.bestTimeForPhotography) fullDesc += 'Best time: ' + desc.bestTimeForPhotography + '\n\n';
    if (desc.visitorPurpose) fullDesc += desc.visitorPurpose;
    
    return fullDesc || 'Photoshoot location in Pondicherry';
  };
  ...
  const renderCategoryCard = ({ item, index }: { item: typeof PHOTOSHOOT_CATEGORIES[0]; index: number }) => (
    <TouchableOpacity
      style={[
        styles.categoryCard,
  ...
  const renderPlaceCard = ({ item, index }: { item: PhotoshootPlace; index: number }) => (
    <TouchableOpacity
      style={[
        styles.placeCard,
        index % 2 === 0 ? styles.cardLeft : styles.cardRight,
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>Loading photoshoot locations...</Text>
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>
          {showCategoryView ? 'Photoshoot Locations' : selectedCategory === 'All' ? 'All Locations' : selectedCategory}
        </Text>
        ...
        <>
          ...
          <View style={styles.resultsBar}>
            <Text style={styles.resultsText}>
              {filteredPlaces.length} {filteredPlaces.length === 1 ? 'location' : 'locations'} found
            </Text>
          </View>
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody} showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.filterOptions}>
                {(['All', ...PHOTOSHOOT_CATEGORIES.map(c => c.id)] as PhotoshootCategory[]).map((cat) => (
                  <TouchableOpacity
                    key={cat}
                    style={[
                      styles.filterOption,
